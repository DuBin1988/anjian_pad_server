...
public class IndoorInspectService {
	...
	public String saveRepairResult(String stringifiedObj) {
		...
		try {
			...
			//更新安检状态
			String sql = "update T_INSPECTION set REPAIR_STATE='已维修', PHOTO_EIGHTH='" + uuid + "_8', " +" REPAIR_DATE='" + dt + "' where id='" + uuid +"'";
			execSQL(sql);
			return "{\"ok\":\"ok\"}";				
		} catch (Exception e) {
			return "{\"ok\":\"nok\"}";
	...
	private void DeletePics(String uuid) {
		...
		{
			hql = "delete from t_blob where id ='" + uuid + "_sign'";
			this.hibernateTemplate.bulkUpdate(hql);
		}
		photo = (String)map.get("PHOTO_FIRST");		
		if(photo == null || photo.length()==0)
		{
			hql = "delete from  t_blob  where id ='" + uuid + "_1'";
			this.hibernateTemplate.bulkUpdate(hql);
		}
		photo = (String)map.get("PHOTO_SECOND");		
		if(photo == null || photo.length()==0)
		{
			hql = "delete from  t_blob  where id ='" + uuid + "_2'";
			this.hibernateTemplate.bulkUpdate(hql);
		}
		photo = (String)map.get("PHOTO_THIRD");		
		if(photo == null || photo.length()==0)
		{
			hql = "delete from  t_blob  where id ='" + uuid + "_3'";
			this.hibernateTemplate.bulkUpdate(hql);
		}
		photo = (String)map.get("PHOTO_FOUTH");		
		if(photo == null || photo.length()==0)
		{
			hql = "delete from  t_blob  where id ='" + uuid + "_4'";
			this.hibernateTemplate.bulkUpdate(hql);
		}
		photo = (String)map.get("PHOTO_FIFTH");	
		if(photo == null || photo.length()==0)
		{
			hql = "delete from  t_blob  where id ='" + uuid + "_5'";
			this.hibernateTemplate.bulkUpdate(hql);
		}
		photo = (String)map.get("PHOTO_SIXTH");	
		if(photo == null || photo.length()==0)
		{
			hql = "delete from  t_blob  where id ='" + uuid + "_6'";
			this.hibernateTemplate.bulkUpdate(hql);
		}photo = (String)map.get("PHOTO_SEVENTH");	
		if(photo == null || photo.length()==0)
		{
			hql = "delete from  t_blob  where id ='" + uuid + "_7'";
			this.hibernateTemplate.bulkUpdate(hql);
		}
	}
	...
	public String CAValidate(@PathParam("newmeter") String newmeter,
		...
		// 安检日期必须大于上次
		try {
			final String sql = "select f_jbdushu, arrival_time from (select f_jbdushu, arrival_time from t_inspection "
					+ " where ROAD ='"
					+ road
					+ "' and UNIT_NAME='"
					+ unit
					...
					+ room
					+ "' and condition='正常'"
					+ " order by arrival_time desc) where ROWNUM<2";
	...
	private boolean CAInsertNewRow(JSONObject row) throws JSONException {
			...
			sql1 += sql2 +")";
			execSQL(sql1);
			sql1 = "update t_inspection set f_anjianriqi= TO_DATE(SUBSTR(arrival_time,1,10), 'yyyy-MM-dd') where id=" + uuid;
			execSQL(sql1);
			if(suggestions != null)
			{
